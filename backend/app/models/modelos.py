...
class PreRegistoObito(Base):
    ...
    hospital_id         = Column(Integer, ForeignKey("hospitais.id"), nullable=False)
    referencia_hospital = Column(String(100), nullable=False)
    bi_falecido         = Column(String(20), nullable=False)
    nome_falecido       = Column(String(200), nullable=False)
    data_obito          = Column(DateTime, nullable=False)
    local_obito         = Column(String(200), nullable=False)
    causa_obito         = Column(String(500), nullable=True)
    nome_declarante     = Column(String(200), nullable=False)
    bi_declarante       = Column(String(20), nullable=False)
    contacto_declarante = Column(String(20), nullable=False)
    email_declarante    = Column(String(200), nullable=True)
    tem_whatsapp        = Column(Boolean, default=False)
    estado              = Column(Enum("aguarda_aprovacao", "aprovado", "rejeitado", "bi_invalido"), default="aguarda_aprovacao")
    motivo_rejeicao     = Column(Text, nullable=True)
    criado_em           = Column(DateTime, default=datetime.utcnow)
    actualizado_em      = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class RegistoObito(Base):
    ...
    nuic_obito       = Column(String(20), unique=True, nullable=False)
    nome_falecido    = Column(String(200), nullable=False)
    bi_falecido      = Column(String(20), nullable=False)
    data_obito       = Column(DateTime, nullable=False)
    local_obito      = Column(String(200), nullable=False)
    ...
    pre_registo = relationship("PreRegistoObito")
...
class NotificacaoLog(Base):
    ...
    canal          = Column(Enum("whatsapp", "email", "pendente"), nullable=False)
    destinatario   = Column(String(200), nullable=False)
    mensagem       = Column(Text, nullable=False)
    com_pdf        = Column(Boolean, default=False)
    estado_envio   = Column(Enum("enviado", "falhou", "pendente"), default="pendente")
